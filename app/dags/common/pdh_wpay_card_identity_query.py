...
def readexecuteQuery(**kwargs):
    ...
    bucket= Variable.get('wpay_card_identity_query',deserialize_json=True)['bucket']
    
    query_file=Variable.get('wpay_card_identity_query',deserialize_json=True)['files']
    execTimeInAest = convertTimeZone(datetime.now(),"UTC","Australia/NSW")
    date_time=execTimeInAest.strftime("%Y-%m-%d %H:%M:%S")
    environment= Variable.get('wpay_card_identity_query',deserialize_json=True)['environment']
    email_to = Variable.get("wpay_card_identity_query", deserialize_json=True)['email_to']
    ...
    for i in query_file:
        ...
        if len(Successfull_execution) > 0:
            subject_success="WPAY Card Identity Model Query executed successfully in "+environment+" "+date_time
...
def processQuery(query,query_file,email,environment,date_time):
    ...
    try:  
        ...
        logging.info("WPAY Card Identity Model Query executed inside processQuery function")
        return True,rows
    except Exception as e:
        logging.info("Exception Raised:{}".format(e))
        logging.info("email_to: {}".format(email))
        complete_exception = str(e)            
        body_fail ="Exception raised while executing WPAY Card Identity Model Query "+query_file+":\n\n"+complete_exception
        subject_fail ="Exception raised while executing WPAY Card Identity Model query in "+environment+" "+date_time
