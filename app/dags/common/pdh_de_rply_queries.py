...
dag = DAG('pdh_de_rply_queries', catchup=False, default_args=default_args,schedule_interval= "00 13 * * *")


def readexecuteQuery(**kwargs):

    bucket= Variable.get('de_reply_query',deserialize_json=True)['bucket']
    
    query_file=Variable.get('de_reply_query',deserialize_json=True)['files']
    execTimeInAest = convertTimeZone(datetime.now(),"UTC","Australia/NSW")
    date_time=execTimeInAest.strftime("%Y-%m-%d %H:%M:%S")
    environment= Variable.get('de_reply_query',deserialize_json=True)['environment']
    email_to = Variable.get("de_reply_query", deserialize_json=True)['email_to']
    ...
    for i in query_file:
        ...
        if len(Successfull_execution) > 0:
            subject_success="DE RPLY queries executed successfully in "+environment+" "+date_time
...
def processQuery(query,query_file,email,environment,date_time):
    ...
    try:  
        ...
        body_fail ="Exception raised while executing DE RPLY queries "+query_file+":\n\n"+complete_exception
        subject_fail ="Exception raised while executing DE RPLY queries in "+environment+" "+date_time
