...
dag_name = "pdh_looker_queries"
try:
    control_table = Variable.get("looker_queries", deserialize_json=True)["control_table"]
...
def readexecuteQuery(**kwargs):
    ...
    bucket= Variable.get('looker_queries',deserialize_json=True)['bucket']
    
    query_file=Variable.get('looker_queries',deserialize_json=True)['files']
    execTimeInAest = convertTimeZone(datetime.now(),"UTC","Australia/NSW")
    date_time=execTimeInAest.strftime("%Y-%m-%d %H:%M:%S")
    environment= Variable.get('looker_queries',deserialize_json=True)['environment']
    email_to = Variable.get("looker_queries", deserialize_json=True)['email_to']
    ...
    if len(Successfull_execution) > 0:        
        event_message = "Looker queries were executed successfully in "+environment+": "+str(Successfull_execution) + " at " + execTimeInAest.strftime("%Y-%m-%d %H:%M:%S")
        event = Event(
            dag_name=dag_name,
...
def processQuery(query,query_file,email,environment,date_time):
    ...
    try:  
        ...
        body_fail ="Exception raised while executing LOOKER queries "+query_file+":\n\n"+complete_exception
        subject_fail ="Exception raised while executing LOOKER queries in "+environment+" "+date_time
        logging.info("email body text: {} ".format(body_fail))
        pu.PDHUtils.send_email(email, subject_fail,body_fail)
        event_message = "Exception raised while executing LOOKER queries "+query_file+":\n\n"+complete_exception
        event = Event(
            dag_name=dag_name,
