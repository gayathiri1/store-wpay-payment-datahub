...
dag = DAG('pdh_file_gen_etl', catchup=False, default_args=default_args,schedule_interval= "00 17,18,19,00 * * *")
...
def readexecuteQuery(**kwargs):
    ...
    for i in query_file:
        ...
        if current_time>=i['start_time'] and current_time<=i['end_time']:
            ...
            result, rows1 = processQuery(qc_query)
            logging.info("Control table record is fetched for file {} and merchant  {}".format(i['query_file'],i['merchant']))
...
def processQuery(query):
    ...
    try:
        ...
        execTimeInAest = convertTimeZone(datetime.now(),"UTC","Australia/NSW")
        subject = "Exception raised while executing file extraction etl"+execTimeInAest.strftime("%Y-%m-%d %H:%M:%S")
        emailTo = Variable.get("file_gen_etl", deserialize_json=True)['emailTo']
        body= "Exception raised while executing file extraction etl: \n"+str(e)
        pu.PDHUtils.send_email(emailTo, subject,body)
        #Senderroremail(emailTo,"Exception raised while executing file extraction etl",body,execTimeInAest.strftime("%Y-%m-%d %H:%M:%S"))
