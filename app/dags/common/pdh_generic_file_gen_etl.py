...
dag = DAG('pdh_generic_file_gen_etl', catchup=False, default_args=default_args, schedule_interval="00 18,19,20,21,01 * * *")


def readexecuteQuery(**kwargs):
    bucket = Variable.get('generic_file_gen_etl', deserialize_json=True)['bucket']

    query_file = Variable.get('generic_file_gen_etl', deserialize_json=True)['files']
    qc_select = Variable.get('generic_file_gen_etl', deserialize_json=True)['qc_select']
    emailTo = Variable.get("generic_file_gen_etl", deserialize_json=True)['emailTo']
    updated_files = []
    unupdated_files = []
    error_count = 0
    updated_query = []
...
triggerdag = TriggerDagRunOperator(
    task_id="triggerdag",
    trigger_dag_id="pdh_generic_file_gen_bq_to_gcs",
    dag=dag,
)
