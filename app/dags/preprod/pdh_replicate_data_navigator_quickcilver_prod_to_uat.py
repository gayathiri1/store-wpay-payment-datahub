...
dag = DAG('pdh_replicate_data_navigator_quickcilver_prod_to_uat', catchup=False, default_args=default_args, schedule_interval= "20 22,23,05 * * *") 
...
def TriggerDML(**kwargs):
    ...
    #project_id_target = Variable.get('data_navigator_quickcilver_pos',deserialize_json=True)['target_project']
    #Config_file = Variable.get('data_navigator_quickcilver_pos',deserialize_json=True)['table']
    project_id_source = Variable.get('data_navigator_quickcilver_pos',deserialize_json=True)['source_project']
    ...
    for i in Config_file:
        ...
        if i['load_type'] == 'I':
            ...
            delete_query.append("delete from `"+project_id_target+"."+i['dataset']+"."+i['name']+'`  where file_date = (select distinct max(file_date) from `'+project_id_source+"."+i['dataset']+"."+i['name']+'` );')		
				
            insert_query.append("insert into `"+project_id_target+"."+i['dataset']+"."+i['name']+"` select * from `"+project_id_source+"."+i['dataset']+"."+i['name']+'` where file_date > (select distinct max(file_date) from `'+project_id_target+"."+i['dataset']+"."+i['name']+'` );')		
            ...
            delete_query.append("delete from `"+project_id_target+"."+i['dataset']+"."+i['name']+'`  where file_date between \''+i['start_date']+'\' and \''+i['end_date']+'\'') 
            insert_query.append("insert into `"+project_id_target+"."+i['dataset']+"."+i['name']+"` select * from `"+project_id_source+"."+i['dataset']+"."+i['name']+'` where file_date between \''+i['start_date']+'\' and \''+i['end_date']+'\'')
...
def executeView(**kwargs):  
    bucket_name = Variable.get("data_navigator_quickcilver_pos", deserialize_json=True)['bucket_name']   
    query_file =  Variable.get("data_navigator_quickcilver_pos", deserialize_json=True)['query_file']    
...
def Sendemail(**kwargs):  
    ...
    subject = "UAT data refresh for Data Navigator and Quickcilver "+loadDateTime
    emailTo = Variable.get("data_navigator_quickcilver_pos", deserialize_json=True)['emailTo']
    logging.info("subject: {} emailto: {}".format(subject,emailTo))
    pu.PDHUtils.send_email(emailTo, subject,'Data Navigator and Quickcilver replication to UAT completed successfully')
    return True
