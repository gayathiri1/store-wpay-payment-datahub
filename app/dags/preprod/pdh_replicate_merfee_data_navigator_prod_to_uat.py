...
dag = DAG('pdh_replicate_merfee_data_navigator_prod_to_uat', catchup=False, default_args=default_args, schedule_interval= "20 22 * * *") 

def getConfigDetails(**kwargs):

    logging.info("getConfigDetails started")
    project_id_target = Variable.get('data_merfee',deserialize_json=True)['target_project']
    Config_file = Variable.get('data_merfee',deserialize_json=True)['table']
    ...
    for i in Config_file:
        ...
        table_id.append(project_id_target+"."+i['dataset']+"."+i['name'])
        if i['encryption'] == 'N':
            encryp_flag[project_id_target+"."+i['dataset']+"."+i['name']] = 'None'
        else:
            encryp_flag[project_id_target+"."+i['dataset']+"."+i['name']]=i['encryption']
        if i['partition_key']== 'N':   
            partition_key[project_id_target+"."+i['dataset']+"."+i['name']] = "None"             
        else:

            partition_key[project_id_target+"."+i['dataset']+"."+i['name']] = i['partition_key']
...
def TriggerDML(**kwargs):
    ...
    #project_id_target = Variable.get('data_merfee',deserialize_json=True)['target_project']
    #Config_file = Variable.get('data_merfee',deserialize_json=True)['table']
    project_id_source = Variable.get('data_merfee',deserialize_json=True)['source_project']
    ...
    for i in Config_file:
        ...
        if i['load_type'] == 'I':
            ...
            delete_query.append("delete from `"+project_id_target+"."+i['dataset']+"."+i['name']+'`  where format_date(\'%Y%m%d\', pdh_load_time) >= format_date(\'%Y%m%d\',DATE_SUB(current_date("Australia/Sydney"), INTERVAL 1 day));')		
				
            insert_query.append("insert into `"+project_id_target+"."+i['dataset']+"."+i['name']+"` select * from `"+project_id_source+"."+i['dataset']+"."+i['name']+'` where format_date(\'%Y%m%d\', pdh_load_time) >= format_date(\'%Y%m%d\',DATE_SUB(current_date("Australia/Sydney"), INTERVAL 1 day));')		
            ...
            delete_query.append("delete from `"+project_id_target+"."+i['dataset']+"."+i['name']+'`  where format_date(\'%Y%m%d\', pdh_load_time) between \''+i['start_date']+'\' and \''+i['end_date']+'\'') 
            insert_query.append("insert into `"+project_id_target+"."+i['dataset']+"."+i['name']+"` select * from `"+project_id_source+"."+i['dataset']+"."+i['name']+'` where format_date(\'%Y%m%d\', pdh_load_time) between \''+i['start_date']+'\' and \''+i['end_date']+'\'')
...
def executeView(**kwargs):  
    bucket_name = Variable.get("data_merfee", deserialize_json=True)['bucket_name']   
    query_file =  Variable.get("data_merfee", deserialize_json=True)['query_file']    
...
def Sendemail(**kwargs):  
    ...
    subject = "UAT data refresh Merfee data replication "+loadDateTime
    emailTo = Variable.get("data_merfee", deserialize_json=True)['emailTo']
    logging.info("subject: {} emailto: {}".format(subject,emailTo))
    pu.PDHUtils.send_email(emailTo, subject,'Merfee data replication to UAT completed successfully')
    return True
...
#executeView = BigQueryOperator(
    #task_id='executeView',
    #sql= Variable.get("data_merfee", deserialize_json=True)['sqlFile'],
